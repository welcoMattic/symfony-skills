...
usage() {
    cat <<EOF
${BOLD}symfony-skills installer${NC}
...
Options:
  --version <X.Y> Symfony version (6.4, 7.4, 8.0). Default: latest (8.0)
...
validate_version() {
    ...
    echo -e "${RED}Error: unsupported Symfony version '$ver'${NC}" >&2
    echo -e "Supported versions: ${BOLD}$SUPPORTED_VERSIONS${NC}" >&2
    exit 1
}

# Resolve skill file path: version override if exists, otherwise base
# Usage: resolve_skill <skill_name>
resolve_skill() {
    local skill="$1"
    if [[ -n "$SYMFONY_VERSION" && -f "$VERSIONS_DIR/$SYMFONY_VERSION/$skill.md" ]]; then
        echo "$VERSIONS_DIR/$SYMFONY_VERSION/$skill.md"
    else
...
install_claude_code() {
    ...
    echo -e "${BLUE}Installing for Claude Code$(version_label)...${NC}"
...
install_opencode() {
    ...
    echo -e "${BLUE}Installing for OpenCode$(version_label)...${NC}"
...
install_codex() {
    ...
    echo -e "${BLUE}Generating AGENTS.md for Codex$(version_label)...${NC}"
...

    {
        ...
        echo ""
        echo "> Auto-generated by [symfony-skills](https://github.com/welcoMattic/symfony-skills)"
        echo ""
        for skill in $(list_skills); do
            cat "$(resolve_skill "$skill")"
            echo ""
...
install_cursor() {
    ...
    echo -e "${BLUE}Generating .cursorrules for Cursor$(version_label)...${NC}"
...
install_windsurf() {
    ...
    echo -e "${BLUE}Generating .windsurfrules for Windsurf$(version_label)...${NC}"
...
install_generic() {
    ...
    echo -e "${BLUE}Generating combined symfony-skills.md$(version_label)...${NC}"
...

    {
        ...
        echo "Generated by [symfony-skills](https://github.com/welcoMattic/symfony-skills)"
        echo ""
        echo "---"
        echo ""
        for skill in $(list_skills); do
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --version)
            SYMFONY_VERSION="$2"
            validate_version "$SYMFONY_VERSION"
            shift 2 ;;
        --project)
            PROJECT=true; shift ;;
        --output)
