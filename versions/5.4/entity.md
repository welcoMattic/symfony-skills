...
- **Annotations for mapping** — use Doctrine annotations (`@ORM\Entity`, `@ORM\Column`). If the project uses PHP 8.0+, attributes are also acceptable, but annotations are the standard in Symfony 5.4
...
- **Validation constraints on the entity** — always add `@Assert\...` annotations directly on properties
...
4. If `--api`, add the `@ApiResource` annotation and ensure API Platform is installed
